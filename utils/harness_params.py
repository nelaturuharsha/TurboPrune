...
def get_current_params() -> None:
    ...
    Section("prune_params", "pruning configuration").params(
        ...
        epoch_schedule_strategy=Param(And(str, OneOf(['linear_increase', 'linear_decrease', 'exponential_decrease', 
    ...
    Section("experiment_params", "parameters to train model").params(
        ...
        compute_metrics = Param(And(str, OneOf(['true', 'false'])), "compute metrics", default='true')
    )
    ...
    Section("optimizer", "data related stuff").params(
        ...
        skip_warmup=Param(And(str, OneOf(['true', 'false'])), "skip warmup", default='false'),
