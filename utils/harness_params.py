...
def get_current_params():
    ...
    Section('prune_params', 'pruning configuration').params(
        ...
        er_method=Param(And(str, OneOf(['er_erk', 'er_balanced', 'synflow', 'snip', 'just dont'])), default='just dont'),
        prune_method=Param(And(str, OneOf(['random_erk', 'random_balanced', 'synflow', 'snip', 'mag'])), required=True),
    ...
    Section('optimizer', 'data related stuff').params(
        ...
        scheduler_type=Param(And(str, OneOf(['MultiStepLRWarmup', 'ImageNetLRDropsWarmup', 'CosineLRWarmup', 'CustomLRScheduler', 'TriangularSchedule'])), required=True),
