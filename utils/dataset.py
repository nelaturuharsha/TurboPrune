...
class CifarLoader:
    def __init__(self, path, train=True, batch_size=500, aug=None, drop_last=None, shuffle=None, altflip=False, dataset='CIFAR10'):
        ...
        data_path = os.path.join(path, f'{dataset}_train.pt' if train else f'{dataset}_test.pt')
...
class AirbenchLoaders:
    ...
    @param('dataset.data_root_dir')
    @param('dataset.batch_size')
    def __init__(self, dataset_name, data_root_dir, batch_size):
        console = Console()
        console.print(Panel("[bold green]Using Airbench CIFAR Loader with small modifications from https://github.com/KellerJordan/cifar10-airbench[/bold green]", border_style="green", expand=False))
        self.train_loader = CifarLoader(path=data_root_dir, batch_size=batch_size, train=True, aug={'flip' : True, 'translate' : 2}, altflip=True, dataset=dataset_name)
        self.test_loader = CifarLoader(path=data_root_dir, batch_size=batch_size, train=False, dataset=dataset_name)
...
class imagenet:
    ...
    def __init__(self, data_root_dir, batch_size, num_workers, this_device, distributed=False):
        ...
        try:
            ...
            console = Console()
            if self.rank == 0:
                panel = Panel("[bold green]FFCV loaded, all good -- you can work on ImageNet![/bold green]", title="FFCV Status", border_style="green", expand=False)
                console.print(panel)
        except ImportError:
            raise ImportError(
        ...
        self.train_loader = Loader(
            os.path.join(data_root_dir, "train_500_0.50_90.beton"),
            batch_size=batch_size,
        ...
        self.test_loader = Loader(
            os.path.join(data_root_dir, "val_500_0.50_90.beton"),
            batch_size=batch_size,
            num_workers=num_workers,
